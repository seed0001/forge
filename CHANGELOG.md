...
- **The agent now commits its own finished work instead of waiting to be asked.** The `git` tool has always existed and behaved safely (read-only commands always run; mutating ones go through the same approval/Plan-mode gate as `run_command`; network subcommands are blocked outright) — but nothing ever told the agent *when* to reach for it, so in practice it only touched git when a message specifically mentioned it. A new GIT COMMIT POLICY, defined once in `agent-service.ts` (`GIT_COMMIT_POLICY`) and shared verbatim the same way `ABOUT_FORGE` is, now spells out real git hygiene: commit at the end of a coherent, working unit of change — not after every small edit, never on something half-finished or untested; check `status`/`diff`/`log` before staging; stage only the files that belong to the change, never a blanket `add -A`; scan the diff for anything that looks like a secret before committing it; write the message around *why*; never amend, force, or skip hooks. It's wired into all three places an agent actually does work: the native tool-calling loop's system prompt, the Codex companion's preamble, and the background builder's brief — each still gated by the Operator's own bash permission (`ask` prompts as normal, `allow` lands silently and logged, `deny` disables git entirely).
...
- **One-time reminders are a real thing now, not just a short recurring interval.** "Remind me in 10 minutes" had nothing to map to before — the scheduler only supported recurring cron/interval schedules, likely why nothing got created in the first place. A new `once` schedule kind fires exactly once, N minutes from now, then removes itself; `schedule_task` (native tool) and Codex's request-file format both gained an `in_minutes` option for it. The native tool loop also gained `list_scheduled_tasks`, for the same "check before answering" reason.

## 0.2.51 — 2026-08-30

- **The agent can create its own scheduled reminders — including on the Codex provider, which couldn't reach the scheduler at all.** Asked for a recurring reminder, the agent had no way to actually create one and just told the Operator to open the Scheduler panel and enter it by hand — even though the scheduler itself has supported cron and interval schedules for a while. A new `schedule_task` tool (label, prompt, and either a cron expression or an interval in minutes) creates a real scheduled task directly, validated the same way the Scheduler panel does. Codex has no access to that tool — it only gets file read/write and shell — so it's instead told to write a small `.forge-schedule-request.json` request file, which goes through the exact same edit review/approval as any other Codex write; Forge's existing 20-second scheduler tick picks it up, creates the task, deletes the file, and posts a confirmation into the chat.

## 0.2.50 — 2026-08-30

- **Codex edits and commands now go through real Operator approval — the same review popup as every other provider.** The 0.2.49 companion/builder split tried to get safety from Codex's own `sandbox_mode` (read-only companion, workspace-write builder) via `codex exec`, but `codex exec` is non-interactive and can never actually pause to ask — so in practice the builder's edits just landed, unreviewed, and the split didn't behave the way it looked like it should. Forge now drives `codex app-server` (`electron/codex-app-server.ts`), a persistent JSON-RPC connection that genuinely suspends a turn and asks before a mutating action. Every Codex thread runs sandboxed **read-only** — confirmed empirically that this is what actually triggers the ask — and those requests are routed through the exact same machinery the native chat model already uses: a per-hunk reviewable diff (`ReviewOverlay`) for edits, the same yes/no popup for commands, gated by the same edit/bash permission levels (`allow`/`ask`/`deny`). Companion and builder still run on separate threads — that split is now purely for letting a build run in the background without blocking the conversation, not a difference in trust. See `COMPANION-ARCHITECTURE.md` for the full design.
