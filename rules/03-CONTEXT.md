...
# Changes made
Counts, not a per-file list: files written, files read, commands run, installs
done (e.g. "3 files written, 5 files read, 1 command run"). Full detail already
lives in AUDIT.md — don't duplicate it here. Name a specific file only if it's
central to Next steps or Open questions.
# Next steps
...
- This is also enforced in code, not just here: at 70% full, agent-service.ts
  auto-replaces older messages with a generated summary before the next request,
  so a session that keeps running doesn't just run out of room. That is a safety
  net, not a substitute — still write to SCRATCH.md yourself so the durable
  record is yours, not whatever the auto-summary happened to keep.
