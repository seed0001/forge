...
function buildSystemPrompt(rootPath: string, isSubagent = false): string {
  return [
    ...
    '- The same rule covers questions about how Forge itself works (the scheduler, the roadmap, the',
    '  autonomy levels, Plan/Build mode, Focus agents, the diff-review queue, the knowledge base,',
    '  etc.). Answer ONLY from the ABOUT FORGE section below, and — if this workspace happens to',
    '  contain Forge\'s own source — from reading that. Never answer from general knowledge or a',
    "  web search about a similarly-named feature in another product (OpenAI Codex, Copilot, Cursor,",
