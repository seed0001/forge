...
export interface ProjectEmit {
  terminal: (projectId: string, evt: TermDataEvent) => void;
  activity: (projectId: string, sessionId: string, evt: ActivityEvent) => void;
  message: (projectId: string, sessionId: string, msg: ChatMessage) => void;
  status: (projectId: string) => void;
  diffProposed: (projectId: string, diff: PendingDiff) => void;
  /** A diff was applied (or otherwise settled) outside the normal decide-in-UI path. */
  diffUpdated: (projectId: string, diff: PendingDiff) => void;
  /** The session list changed — new session, rename, or a saved update. */
  sessions: (projectId: string) => void;
  ...
  subagentCommandApproval: (projectId: string, req: SubagentCommandApproval) => void;
  /** A session's whole roadmap, sent fresh on every change (propose/decide/edit/push-back/status). */
  roadmapUpdated: (projectId: string, sessionId: string, items: RoadmapItem[]) => void;
  /** This workspace's whole scheduled-task list, sent fresh on every create/update/delete/fire. */
  schedulerUpdated: (projectId: string, tasks: ScheduledTask[]) => void;
  /** This workspace's whole Focus agent list, sent fresh whenever one starts, finishes, or is stopped. */
  focusUpdated: (projectId: string, agents: FocusAgentSummary[]) => void;
  /** The shared cross-agent message board, sent fresh on every post. */
  focusBoardUpdated: (projectId: string, messages: FocusMessage[]) => void;
  /** An ask_and_wait call is waiting on the Operator (or a peer agent's reply) — see requestFocusAnswer. */
  focusQuestion: (projectId: string, req: FocusQuestion) => void;
}

/**
 * A Project's link to its PARENT Workspace — set by WorkspaceManager.addProject
 * at construction, this is what lets every AgentSession this project creates
 * see the workspace-wide knowledge base/meta-file/sibling-project list on top
 * of its own project-level context, without the Project needing to hold a
 * reference to the whole Workspace object (which would create a Project <->
 * Workspace <-> Project reference cycle for no benefit — this is the only
 * slice of the Workspace a Project ever actually needs).
 */
export type ProjectWorkspaceLink = WorkspaceContext;
...
/**
 * A Project is a self-contained unit of work: its own folder, terminal, and
 * review queue, shared across every session in it (they're all editing the
 * same project on disk) — but each session gets its own independent agent
 * conversation and running state (see SessionRuntime). This is what used to
 * be called a "Workspace" before a new, higher-level Workspace (see
 * electron/workspace.ts) was inserted above it as a renameable, typed group
 * of Projects; a Project's own behavior here is unchanged by that split.
 *
 * Everything here lives in the main process and is driven by the agent loop, so
 * a task keeps running after the user switches to a different session, project
...
export class Project {
  ...
  private lineSeq = 0;
  /** Link to this project's PARENT Workspace — its workspace-scoped ContextStore, meta-file text, and sibling-project listing, injected into every AgentSession this project creates. Undefined only for a Project constructed outside the normal WorkspaceManager.addProject path (should not happen in practice). */
  private workspaceLink?: ProjectWorkspaceLink;

  constructor(id: string, name: string, rootPath: string | null, emit: ProjectEmit, workspaceLink?: ProjectWorkspaceLink) {
    this.id = id;
    this.name = name;
